...
which implements a non-parametric estimator of the cumulative incidence function
for competing risks.
See the :ref:`user guide on the analysis of competing risks </user_guide/competing-risks.ipynb>`.

Bug fixes
^^^^^^^^^
- In the C++ code of :class:`sksurv.linear_model.CoxnetSurvivalAnalysis`, set the type of ``n_alphas``
