...
We highly recommend saying "Hi!" to us before you start working on scikit-survival.
...
Just post a message in `GitHub Discussions <https://github.com/sebp/scikit-survival/discussions>`_.
If you're not sure what to write, here's a helpful template:
...
All code in scikit-survival is released under the `GNU General Public License v3.0 <https://github.com/sebp/scikit-survival/blob/main/COPYING>`_ (GPL-3).
Contributors license their code under the same license when it is included in scikit-survival's version control repository.
That means you must own the copyright of any code submitted to scikit-survival.
...
Code generated by AI may infringe on copyright and it is the submitter's responsibility to not infringe.

Use of Generative AI
--------------------

Contributors using generative AI tools (for example Claude Code) to prepare contributions must
follow the :ref:`ai_guidelines` and declare such use in the ``AI Disclosure`` section of the pull request template.
...
3. Each function, class, method, and attribute needs to be documented using docstrings.
   ...
   To validate docstrings, install `tox`_ and run::
...
Part of the codebase is written in `Cython`_. To rebuild this code after making changes,
