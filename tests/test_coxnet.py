...
def assert_predictions_equal(coxnet, x, expected_pred):
    pred = np.array([coxnet.predict(x.iloc[122:123, :], alpha=a)[0] for a in coxnet.alphas_])
    ...
    def data_example_2_normalize(self):
        ...
        expected_alphas = np.array(
            [
                ...
                0.00117917939332615,
                0.00107442434597765,
                0.000978975448318574,
                0.000892005967659349,
                0.000812762615963956,
                0.000740559025229353,
                0.000674769802494178,
                0.000614825112984099,
                0.000560205744475609,
                0.000510438610128125,
                0.000465092650832102,
                0.000423775101581237,
                0.000386128089529882,
                0.000351825534269655,
                0.000320570323476944,
                0.000292091739468086,
                0.000266143114372309,
                0.000242499693612633,
                0.000220956689189248,
                0.000201327505903829,
                0.00018344212516119,
                0.000167145632349552,
                0.000152296875044284,
                0.000138767240413126,
                0.000126439541233365,
                0.000115207000872171,
                0.000104972328438487,
                9.5646876095872e-05,
                8.71498712373515e-05,
                7.94077168717352e-05,
                7.23533541616886e-05,
                6.59256815921643e-05,
                6.00690257383086e-05,
                5.47326590488896e-05,
            ]
        )
    ...
    def data_example_2_with_n_alpha_and_norm(self):
        ...
        expected_alphas = np.array(
            [
                ...
                0.00104728462934176,
                0.00052488568620016,
                0.000263066005037211,
                0.000131845323325978,
                6.607919286444e-05,
                3.31180478720517e-05,
                1.65983427961291e-05,
...
def test_coxnet_fit_and_predict(params, x, y, expected):
    ...
    coef = pd.DataFrame(coxnet.coef_[:, : len(expected.alphas)], columns=expected.coef.columns, dtype=float)
...
class TestCoxnetSurvivalAnalysis:
        ...
        expected_pred = (
            np.array(
                [
                    ...
                    [
                        0,
                        -1.68697383231657e-05,
                        -9.72026227669121e-05,
                        -0.000177535507210586,
                        0.0889321572547518,
                        0.161856892574037,
                    ],
                ]
    ...
    def test_predict_func_disabled(self):
        x, coxnet = self._fit(l1_ratio=0.9, n_alphas=11, alpha_min_ratio=0.001, fit_baseline_model=False)
        with pytest.raises(ValueError, match="`fit` must be called with the fit_baseline_model option set to True."):
    ...
    def test_invalid_max_iter(self, invalid_positive_int):
        msg = (
            "The 'max_iter' parameter of CoxnetSurvivalAnalysis must be an int "
    ...
    def test_invalid_n_alphas(self, invalid_positive_int):
        msg = (
            "The 'n_alphas' parameter of CoxnetSurvivalAnalysis must be an int "
            r"in the range \[1, inf\)\. "
            f"Got {invalid_positive_int} instead\\."
        )
    ...
    def test_negative_penalty_factor_value(self, negative_float_array):
        with pytest.raises(ValueError, match="Negative values in data passed to penalty_factor"):
    ...
    def test_infinite_penalty_factor_value(self, infinite_float_array):
        with pytest.raises(ValueError, match="Input penalty_factor contains infinity or a value too large"):
    ...
    def test_alpha_too_small(breast_cancer):
        ...
        with pytest.raises(
            ArithmeticError, match="Numerical error, because weights are too large. Consider increasing alpha."
        ):
            nn.fit(Xf, yf)
    ...
    def test_breast_example(breast_cancer):
        ...
        expected_alphas = np.array(
            [
                ...
                0.00113486973808373,
                0.00103405103838443,
                0.000942188794098442,
                0.000858487338411865,
                0.000782221689357606,
                0.00071273127036839,
                0.000649414188678556,
                0.000591722022016858,
                0.00053915506843511,
                0.00049125801812897,
                0.000447616009762226,
                0.000407851037136367,
                0.000371618675081733,
                0.000338605096211458,
                0.000308524352698783,
                0.00028111589953377,
                0.000256142337807075,
                0.000233387358474159,
                0.000212653868789829,
                0.000193762285185162,
                0.000176548977800548,
                0.000160864853202119,
                0.000146574063005757,
                0.000133552827223371,
                0.000121688362139862,
                0.000110877903434536,
                0.000101027816085719,
                9.20527833489927e-05,
                8.38750677843702e-05,
                7.64238379317803e-05,
                6.96345548028444e-05,
                6.34484128750348e-05,
            ]
        )
    ...
    def test_breast_p_greater_n(breast_cancer):
        ...
        expected_alphas = np.array(
            [
                ...
                0.00109030182243944,
                0.00104074592190515,
                0.000993442413531659,
                0.000948288922619052,
            ]
        )
...
def test_pipeline_predict(breast_cancer, func):
    ...
    pipe = make_pipeline(
        OneHotEncoder(), CoxnetSurvivalAnalysis(alpha_min_ratio=0.0001, l1_ratio=1.0, fit_baseline_model=True)
    )
