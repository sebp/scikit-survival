...
def negative_float_array(request):
    a, b = request.param
    penalty = a * np.ones(30, dtype=float)
    penalty[11] = b
    return penalty
...
def assert_columns_almost_equal(actual, expected, decimal=6):
    for i, col in enumerate(expected.columns):
        assert_array_almost_equal(
            expected.loc[:, col].values, actual.loc[:, col].values,
            decimal=decimal,
...
class CoxnetCases(FixtureParameterFactory):
    ...
    def get_example():
        return get_x_y(
            pd.read_csv(EXAMPLE_FILE),
            ["status", "time"],
            pos_label=1,
        )

    @staticmethod
    def get_example_coef(kind):
        return pd.read_csv(EXAMPLE_COEF_FILE.format(kind))

    def data_example_1(self):
        x, y = self.get_example()
        kwargs = {"alpha_min_ratio": 0.0001, "l1_ratio": 0.5}
        ...
        expected_coef = self.get_example_coef(1)
        ...
        return kwargs, x, y, expected

    def data_example_1_penalty_factor_1(self):
        x, y = self.get_example()

        pf = np.ones(30)
        pf[4] = 0.125
        kwargs = {"alpha_min_ratio": 0.0001, "l1_ratio": 0.5, "penalty_factor": pf}
        ...
        expected_coef = self.get_example_coef("1-pf")
    ...
    def data_example_1_penalty_factor_2(self):
        x, y = self.get_example()
        ...
        expected_coef = self.get_example_coef("1-pf2")
    ...
    def data_example_1_unpenalized(self):
        x, y = self.get_example()
        ...
        expected_coef = self.get_example_coef("1-unpen")
    ...
    def data_example_2(self):
        x, y = self.get_example()

        kwargs = {"alpha_min_ratio": 0.0001, "l1_ratio": 0.9}
        ...
        expected_coef = self.get_example_coef(2)
    ...
    def data_example_2_normalize(self):
        x, y = self.get_example()

        kwargs = {"alpha_min_ratio": 0.0001, "l1_ratio": 0.9, "normalize": True}
        ...
        expected_coef = self.get_example_coef("2-norm")
    ...
    def data_example_2_standardize(self):
        from sklearn.preprocessing import StandardScaler

        x, y = self.get_example()

        scaler = StandardScaler()
        x_t = pd.DataFrame(scaler.fit_transform(x), columns=x.columns, index=x.index)
        ...
        return kwargs, x_t, y, expected

    def data_example_2_with_alpha(self):
        x, y = self.get_example()

        expected_alphas = np.array([0.45, 0.4, 0.35, 0.25, 0.1, 0.05, 0.001])

        kwargs = {"alpha_min_ratio": 0.0001, "l1_ratio": 0.9, "alphas": expected_alphas, "normalize": True}

        expected_coef = self.get_example_coef("2-alpha")

        expected_pred = np.array([0, 0, 0, 0, 0, 0, -0.25599344616167])
    ...
    def data_example_2_with_n_alpha_and_norm(self):
        x, y = self.get_example()
        ...
        kwargs = {"l1_ratio": 0.9, "n_alphas": 11, "alpha_min_ratio": 0.001, "normalize": True}

        expected_coef = self.get_example_coef("2-nalpha-norm")
    ...
    def data_simple(self):
        ...
        kwargs = {"alpha_min_ratio": 0.0001, "l1_ratio": 1.0}
        ...
        expected_coef = pd.read_csv(SIMPLE_COEF_FILE, header=None, skiprows=1)
...
@pytest.mark.parametrize("params,x,y,expected", CoxnetCases().get_cases())
def test_coxnet_fit_and_predict(params, x, y, expected):
    coxnet = CoxnetSurvivalAnalysis(**params)
    coxnet.fit(x, y)

    coef = pd.DataFrame(
        coxnet.coef_[:, :len(expected.alphas)], columns=expected.coef.columns, dtype=float
    )
    assert_columns_almost_equal(coef, expected.coef)

    assert_array_almost_equal(expected.alphas, coxnet.alphas_[:len(expected.alphas)])
    ...
    if expected.pred is not None:
        assert_predictions_equal(coxnet, x, expected.pred - expected_offset)


@pytest.mark.parametrize("normalize_options", [False, True])
class TestCoxnetBaselineModel:
    def _fit(self, X, y, normalize_options):
        X_test = X.iloc[:19]
        X_train = X.iloc[19:]
        y_train = y[19:]
        ...
        return coxnet_1, coxnet_2, X_test, X_test_t
...
class TestCoxnetSurvivalAnalysis:
    def _fit(self, **kwargs):
        x, y = CoxnetCases.get_example()
        coxnet = CoxnetSurvivalAnalysis(**kwargs)
        coxnet.fit(x, y)

        return x, coxnet

    def test_example_2_with_n_alpha_interpolation(self):
        x, coxnet = self._fit(l1_ratio=0.9, n_alphas=11, alpha_min_ratio=0.001)
        ...
        expected_coef = CoxnetCases.get_example_coef("2-nalpha")
        coef = pd.DataFrame(coxnet.coef_, columns=expected_coef.columns, dtype=float)
    ...
    def test_example_2_predict_func(self):
        x, coxnet = self._fit(
            l1_ratio=0.9, n_alphas=11, alpha_min_ratio=0.001, fit_baseline_model=True
        )
    ...
    def test_predict_func_disabled(self):
        x, coxnet = self._fit(
            l1_ratio=0.9, n_alphas=11, alpha_min_ratio=0.001, fit_baseline_model=False
        )
    ...
    def test_predict_func_no_such_alpha(self):
        ...
        for a in 1. + np.random.randn(100):
            with pytest.raises(ValueError,
    ...
    def test_all_zero_coefs(self):
        alphas = np.array([256, 128, 96, 64, 48])

        with pytest.warns(UserWarning, match="all coefficients are zero, consider decreasing alpha."):
            _, coxnet = self._fit(l1_ratio=0.9, alphas=alphas, alpha_min_ratio=0.001)
        assert_array_almost_equal(coxnet.coef_, np.zeros((30, 5), dtype=float))
    ...
    def test_invalid_penalty_factor_length(self, length):
        ...
        with pytest.raises(ValueError, match=msg):
            self._fit(alpha_min_ratio=0.0001, penalty_factor=array)

    def test_negative_penalty_factor_value(self, negative_float_array):
        with pytest.raises(ValueError,
                           match="Negative values in data passed to penalty_factor"):
            self._fit(alpha_min_ratio=0.0001, penalty_factor=negative_float_array)

    def test_infinite_penalty_factor_value(self, infinite_float_array):
        with pytest.raises(ValueError,
                           match="Input penalty_factor contains infinity or a value too large"):
            self._fit(alpha_min_ratio=0.0001, penalty_factor=infinite_float_array)

    def test_nan_penalty_factor_value(self, nan_float_array):
        with pytest.raises(ValueError,
                           match="Input penalty_factor contains NaN"):
            self._fit(alpha_min_ratio=0.0001, penalty_factor=nan_float_array)

    def test_negative_alphas(self, negative_float_array):
        with pytest.raises(ValueError,
                           match="Negative values in data passed to alphas"):
            self._fit(alpha_min_ratio=0.0001, alphas=negative_float_array)

    def test_infinite_alphas(self, infinite_float_array):
        with pytest.raises(ValueError,
                           match="Input alphas contains infinity or a value too large"):
            self._fit(alpha_min_ratio=0.0001, alphas=infinite_float_array)

    def test_nan_alphas(self, nan_float_array):
        with pytest.raises(ValueError,
    ...
    def test_breast_example(breast_cancer):
        ...
        expected_deviance_ratio = np.array([
        ...
        coef = pd.DataFrame(coxnet.coef_, index=x.columns, dtype=float)
        expected_coef = pd.read_csv(BREAST_CANCER_COEFFICIENTS_FILE, index_col=0)
        expected_coef.columns = np.arange(expected_coef.shape[1])
    ...
    def test_breast_p_greater_n(breast_cancer):
        ...
        coef = pd.DataFrame(coxnet.coef_, index=x.columns, dtype=float)
        expected_coef = pd.read_csv(BREAST_CANCER_HIGH_COEFFICIENTS_FILE, index_col=0)
        expected_coef.columns = np.arange(expected_coef.shape[1])
