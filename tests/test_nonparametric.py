...
class CGVHD_DataSets(FixtureParameterFactory):
    ...
    def data_full(self):
        ...
        true_ci = np.empty(shape=(2, 3, 75), dtype=cix.dtype)
        true_ci[0], true_ci[1] = cix[0:3], cix[3:]
...
class TestCumIncCompetingRisks:
    ...
    @staticmethod
    @pytest.mark.parametrize("event, time, true_x, true_y, true_ci", CGVHD_DataSets().get_cases())
    def test_ci(event, time, true_x, true_y, true_ci):
        x, y, ci = cumulative_incidence_competing_risks(event, time, conf_type="log-log", var_type="Aalen")

        assert_array_equal(x, true_x)
        _x_km, true_km, true_ci_0 = kaplan_meier_estimator(event > 0, time, conf_type="log-log")
        assert_array_almost_equal(y[0], 1 - true_km)
        assert_array_almost_equal(y[1:], true_y)
        assert_array_almost_equal(y[0], y[1:].sum(axis=0))

        assert_allclose(ci[:, 0, :], 1 - true_ci_0, rtol=5.0e-4)

        ci_1 = ci[:, 1:, :]
        nan_mask = np.isnan(true_ci)
        assert_allclose(np.ma.masked_array(ci_1, nan_mask), np.ma.masked_array(true_ci, nan_mask), rtol=5.0e-4)
        assert_array_equal(np.ma.masked_array(ci_1, ~nan_mask), np.ma.masked_array(np.zeros_like(ci_1), ~nan_mask))

    @staticmethod
    @pytest.mark.parametrize("event, time, true_x, true_y, true_ci", CGVHD_DataSets().get_cases())
    def test_scrucca_ci(event, time, true_x, true_y, true_ci):
    ...
    def test_invalid_conf_level_competing_risks(event, time, true_x, true_y, conf_level):
        ...
        with pytest.raises(ValueError, match=re.escape(msg)):
            cumulative_incidence_competing_risks(
                event, time, conf_level=conf_level, conf_type="log-log", var_type="Dinse_Approx"
            )
    ...
    def test_invalid_conf_type_competing_risks(event, time, true_x, true_y, conf_type):
        ...
        with pytest.raises(ValueError, match=msg):
            cumulative_incidence_competing_risks(
                event, time, conf_level=0.9, conf_type=conf_type, var_type="Dinse_Approx"
            )
