...
class CoxPHSurvivalAnalysis(BaseEstimator, SurvivalAnalysisMixin):
	...
	def _trapz_wrapper(self, X):
		"""Helper function

		TBC
		"""
		# NOTE: y - values | x - index
		return trapz(X.y, X.x)
	...
	def predict_expectation(self, X):
		...
		TBC
		"""
		sf = self._baseline_model.get_survival_function(self.predict(X))
		return numpy.array(list(map(self._trapz_wrapper, sf)))