...
#include "coxnet/group_penalty.h"
        ...
        bool verbose)
{
    ...
    typedef Eigen::Map<S> VectorType;
    typedef Eigen::Map<U> IntVectorType;
    typedef coxnet::GroupPenalty<MatrixType, VectorType, IntVectorType> CoxnetType;
    typedef typename CoxnetType::DataType DataType;
    typedef typename CoxnetType::SizeVectorType SizeVectorType;
    typedef coxnet::FitResult<MatrixType, VectorType> ResultType;
    ...
    IntVectorType event_map(create_map<U> (event));
    VectorType pen_map(create_map<S> (penalty_factor));
    SizeVectorType groups_map(create_map<Eigen::Matrix<std::int32_t, Eigen::Dynamic, 1> > (groups));

    const DataType _data(x_map, time_map, event_map, pen_map, groups_map);
